const fieldClass =
  "w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition placeholder:text-gray-400 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 dark:border-white/10 dark:bg-[#0b0b0f] dark:text-white dark:placeholder:text-white/40";
...
const ProfileForm = ({
  ...
  return (
    <form
      onSubmit={onSave}
      className="rounded-[24px] border border-gray-200 bg-white p-6 dark:border-white/10 dark:bg-[#14141a] sm:p-8"
    >
      <div className="border-b border-gray-200 pb-6 dark:border-white/10">
        ...
        <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid gap-1 text-sm text-gray-500 dark:text-white/65 sm:text-right">
            <p className="font-medium text-gray-900 dark:text-white">{displayName}</p>
            <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-white/40">
              {roleLabel}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          <button
            ...
            className="rounded-full border border-gray-300 px-4 py-2 text-sm font-medium text-gray-600 transition hover:border-gray-400 hover:text-gray-900 disabled:cursor-not-allowed disabled:opacity-40 dark:border-white/20 dark:text-white/75 dark:hover:border-white/40 dark:hover:text-white"
          >
            Reset
          </button>
          <button
      ...
      <div className="mt-6 grid gap-5 md:grid-cols-2">
        <label className="space-y-2">
          <span className="text-sm font-medium text-gray-600 dark:text-white/75">First name</span>
          <input name="first_name" value={form.first_name} onChange={onChange} placeholder="First name" className={fieldClass} />
        </label>
        <label className="space-y-2">
          <span className="text-sm font-medium text-gray-600 dark:text-white/75">Last name</span>
          <input name="last_name" value={form.last_name} onChange={onChange} placeholder="Last name" className={fieldClass} />
        </label>
        <label className="space-y-2 md:col-span-2">
          <span className="text-sm font-medium text-gray-600 dark:text-white/75">Username</span>
          <input name="username" value={form.username} onChange={onChange} placeholder="username" className={fieldClass} />
        </label>
        <label className="space-y-2 md:col-span-2">
          <span className="text-sm font-medium text-gray-600 dark:text-white/75">Short bio</span>
        ...
        <label className="space-y-2 md:col-span-2">
          <span className="text-sm font-medium text-gray-600 dark:text-white/75">Profile description</span>
