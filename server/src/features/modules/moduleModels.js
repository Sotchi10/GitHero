...
export const getAvailableModules = async (userId) => {
  const [rows] = await db.query(`
    ...
    FROM modules m
    LEFT JOIN lessons l ON l.module_id = m.module_id AND l.is_published = 1
...
export const getAvailableLesson = async (lessonId) => {
  const [rows] = await db.query(`
    ...
    WHERE l.lesson_id = ? AND l.is_published = 1 AND m.is_published = 1`, [lessonId]);
  return rows[0] || null;
};
...
export const getDashboardSummary = async (userId) => {
  const [[summary], [completedModuleRows], [nextRows]] = await Promise.all([
    db.query(`
    ...
    db.query(`
      ...
      FROM (
        SELECT m.module_id
        FROM modules m
        JOIN lessons l ON l.module_id = m.module_id AND l.is_published = 1
        LEFT JOIN user_lesson_progress ulp ON ulp.lesson_id = l.lesson_id AND ulp.user_id = ?
        WHERE m.is_published = 1
